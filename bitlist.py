...
def read_G(traj_num, depth, version='old'):
    if version == 'new':
        sdir = './data/'
    elif version == 'old':
        sdir = './data/_backup/'
    files = os.listdir(sdir)
    file_name = ('G_%d_%d.pickle' % (traj_num, depth))
    path = sdir+file_name
    if file_name in files:
        ...
        d = pd.read_pickle(sdir+'Data_%s_%d.pickle' % ('short', traj_num))
...
def read_AG(traj_num, depth, version):
    if version == 'new':
        sdir = './data/'
    elif version == 'old':
        sdir = './data/_backup/'
    files = os.listdir(sdir)
    file_name = ('AG_%d_%d.npy' % (traj_num, depth))
    path = sdir+file_name
    if file_name in files:
        A = np.load(path)
    else:
        time_interval, G = read_G(traj_num, depth)
        ...
        for i in range(len(l)):
            for t in l[i]:
                A[i][t] = 1
        np.save(sdir+('AG_%d_%d.npy' % (traj_num, depth)), A)
    return A
...
def read_Bl(traj_num, index_para, base, index_type, dtype, version, max_depth):
    if index_type[0] == 'q':
        ...
        A = read_AG(traj_num, index_para, dtype, version)
